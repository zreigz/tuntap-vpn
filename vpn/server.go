...
type VpnServer struct {
	...
	pktHandle    map[byte](func(*udpPacket, *VpnPacket))
...
func NewServer(cfg ServerConfig) error {
	...
	vpnServer._chanBufSize = 2048
	vpnServer.fromNet = make(chan *udpPacket, vpnServer._chanBufSize)
	vpnServer.fromIface = make(chan []byte, vpnServer._chanBufSize)
	vpnServer.toIface = make(chan *VpnPacket, vpnServer._chanBufSize)
	vpnServer.peers = make(map[uint64]*VpnPeer)
	vpnServer.cfg = cfg
	vpnServer.toNet = make([]chan *udpPacket, (cfg.PortEnd - cfg.PortStart + 1))
	...
	go vpnServer.forwardFrames()

	go vpnServer.cleanUp()

	// serve for multiple ports
	for idx, port := 0, cfg.PortStart; port <= cfg.PortEnd; port++ {
		go vpnServer.listenAndServe(cfg.ListenAddr, fmt.Sprintf("%d", port), idx)
		idx++
	}

	go vpnServer.peerTimeoutWatcher()
	logger.Debug("Recieving iface frames")

	go func() {
		for {
			hp := <-vpnServer.toIface
	...
	for {
		...
		vpnServer.fromIface <- hpbuf
	}

}

func (srv *VpnServer) listenAndServe(addr string, port string, idx int) {
	port = addr + ":" + port
...
func (srv *VpnServer) forwardFrames() {

	// packet map
	srv.pktHandle = map[byte](func(*udpPacket, *VpnPacket)){
...
func (srv *VpnServer) handlePacket(packet *udpPacket) {
	defer func() {
	...
	hPack := unpackHopPacket(packet.data)
...
func (srv *VpnServer) toClient(peer *VpnPeer, flag byte, payload []byte, noise bool) {
	hp := new(VpnPacket)
	hp.Seq = peer.Seq()
	hp.Flag = flag
	hp.payload = payload
...
func (srv *VpnServer) bufferToClient(peer *VpnPeer, buf []byte) {
	hp := new(VpnPacket)
	hp.Flag = HOP_FLG_DAT
	hp.buf = buf
...
func (srv *VpnServer) handleKnock(u *udpPacket, hp *VpnPacket) {
...
func (srv *VpnServer) handleHeartbeatAck(u *udpPacket, hp *VpnPacket) {
	sid := uint64(binary.BigEndian.Uint32(hp.payload[:4]))
	sid = (sid << 32) & uint64(0xFFFFFFFF00000000)
...
func (srv *VpnServer) handleHandshake(u *udpPacket, hp *VpnPacket) {
	sid := uint64(binary.BigEndian.Uint32(hp.payload[:4]))
	sid = (sid << 32) & uint64(0xFFFFFFFF00000000)
...
func (srv *VpnServer) handleHandshakeAck(u *udpPacket, hp *VpnPacket) {
	sid := uint64(binary.BigEndian.Uint32(hp.payload[:4]))
	sid = (sid << 32) & uint64(0xFFFFFFFF00000000)
	hpeer, ok := srv.peers[sid]
	if !ok {
...
func (srv *VpnServer) handleDataPacket(u *udpPacket, hp *VpnPacket) {
	sid := uint64(hp.Sid)
	sid = (sid << 32) & uint64(0xFFFFFFFF00000000)
...
func (srv *VpnServer) handleFinish(u *udpPacket, hp *VpnPacket) {
	sid := uint64(binary.BigEndian.Uint32(hp.payload[:4]))
	sid = (sid << 32) & uint64(0xFFFFFFFF00000000)
...
func (srv *VpnServer) kickOutPeer(sid uint64) {
	hpeer, ok := srv.peers[sid]
	if !ok {
		return
	}
...
func (srv *VpnServer) deletePeer(sid uint64) {
	hpeer, ok := srv.peers[sid]
	if !ok {
		return
	}
...
func (srv *VpnServer) peerTimeoutWatcher() {
